...
public class Main2_JSON {
	...
	public static void m2() {
		...
		JSONObject obj = new JSONObject(sb.toString()); // file객체를 이용해서 json object를 만드는 메소드가 없음. new JSONObject(Stirng
														// source) 를 사용, api1.json를 읽어드려서 String에 저장.

		// {"프로퍼티", 값}
		// {"프로퍼티영어로", "value"}

		// 현재 obj의 프로퍼티 값은 각 1개씩 존재.

		obj.getJSONObject("response"); // response의 value를 호출함
	...
	public static void m4() {
		...
		for(int i =0; i<item.length(); i++) {
			JSONObject element = item.getJSONObject(i);	//pty???
			System.out.println(element.getString("category")+" : "+element.getString(null));
		}
