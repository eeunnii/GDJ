	...
	public static void main(String[] args) {
		...
		try {
			...
			for (int i = 0; i < boxOfficeList.getLength(); i++) {
				...
				Movie movie = Movie.builder().movieCd(movieCd).movieNm(movieNm).openDt(openDt).salesAcc(salesAcc)
						.audiAcc(audiAcc).build(); // xml에 필요한 객체가 자바가 됐음 // 이런 자바 객체를 POJO(데이터만 담고 있는 순수한 객체?)라고 함
