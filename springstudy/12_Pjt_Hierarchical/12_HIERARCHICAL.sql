...
STATE NUMBER(1) NOT NULL,   -- 정상 게시글은 1로 하고 삭제된 게시글은 
DEPTH NUMBER(2) NOT NULL,   -- 원글 0, 1차 댓글 : 1, 2차 댓글 : 2, ....
GROUP_NO NUMBER NOT NULL,   -- 원글과 모든 댓글은 같은 GROUP_NO, 원글 : BBS_NO, 댓글 : 원글에 BBS_NO
-- 원글과 원글 밑에 달린 댓글들은 하나의 그룹으로 처리함. GROUP_NO가 이러한 그룹의 번호 
-- 그룹으로 처리하는 이유 : 정렬을 위해서 
...
commit;
	...
	INSERT INTO BBS (BBS_NO, WRITER ,TITLE ,IP ,CREATE_DATE ,STATE ,DEPTH ,GROUP_NO ,GROUP_ORDER)
			VALUES (BBS_SEQ.NEXTVAL, #{title}, #{writer}, #{ip}, SYSDATE, 1, 0, BBS_SEQ.CURRVAL, 0)
