...
--2) SELECT GET_STUDENT('jun123') AS 학번
FROM STUDENT;
...
-- PROFNO(P) - PROFNO(F)                    :조인조건 1
--             DEPTNO(P) - DEPTNO(F)        :조인조건 2
--                         STUDNO(P)

-- 생성순서 ㅣ PROFESSOR -> DEPARTMENT -> STUDENT
...
--1.STUENDT 테이블에서 DEPTNO=101 정보 지우기 
--2.DEPARTMENT 테이블에서 DEPT-101인 PROFNO 저장하기 
--3.DEPARTMENT 테이블에서 DEPT-101인 정보 지우기 
--4.PROFESSOR 테이블에서 2단계에서 저장한 PROFNO와 동일한 정보 지우기 



--1)정의
CREATE OR REPLACE PROCEDURE MY_PROC(DNO IN DEPARTMENT.DEPTNO%TYPE)   -- 프로시저인경우 입력인지 출력파라미터인지 알려줘야함.
IS
    PNO PROFESSOR.PROFNO%TYPE;
BEGIN
    DELETE FROM STUDENT WHERE DEPTNO = DNO;
    SELECT PROFNO INTO PNO FROM DEPARTMENT WHERE DEPTNO = DNO;          -- PROFESSOR 넘버를 PNO에 저장하라. 왜지?
...
SELECT S.SNAME, D.DNAME, P.PNAME
    FROM DEPARTMENT D 
    JOIN STUDENT S ON D.DEPTNO = S.DEPTNO
    JOIN professor P ON P.PROFNO = D.PROFNO;