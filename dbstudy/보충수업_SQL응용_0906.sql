...
-- 프로시저 작성
--학과번호 (DEPTNO) 전달하면 소속 학생, 교수, 학과 모두 제거하는 
--프로시저 실행은 학과번호=101 학과를 대상으로 함
--프로시저이름은 MY_PORC
--예외 발생 시 예외코드 출력+예외메시지 출력+ROLLBACK

CREATE OR REPLACE PROCEDURE MY_PORC(DNO IN DEPARTMENT.DEPTNO%TYPE)
IS
BEGIN
...
EXECUTE MY_PORC(101);   -- IN 앞의 DNO에 값이 들어감
...
RETURN NUMBER  -- 반환하는 학번의 타입이 NUMBER(5)이므로 (주석달아서 시험지 내면 틀린거로 한다고 방금쌤이그럼심)
IS
...
--트리거 
--SAMPLE 테이블이 행을 갱신, 삭제 이후 'SAMPLETRG동작' 서버메시지 출력하기 
