...
-- 2. 길이 
--     1)LEGNTH(칼럼)  : 칼럼 데이터의 글자 수 반환
--     2)LEGHTHB(칼럼) : 칼럼 데이터의 바이트 수 반환 ★시험대비
SELECT
...
-- 3. 연결
--      1) || 연산자
--            (1) CONCAT함수의 인수(ARGUMENTS)는 2개만 지원
--            (2) A와 B와 C의 연결
--                  ① CONCAT(A,B,C)          : 불가능함
...
--      칼럼 데이터의 BEGIN위치부터 LENGTH개 만큼 반환 - BEGIN 인덱스가 아니다. BEGIN은 0부터 시작하지 않고 1부터 시작한다 .
SELECT 
